...
#if S_FUNCTION_LEVEL == 2 && defined(MDL_FINALIZE)
  static void __mdlFinalize(SimStruct *S)
  {
      REQUIRE_LOCAL_STACK_FRAME;
      mdlFinalize(S);
  }
...
#if S_FUNCTION_LEVEL == 2 && defined(MDL_FINALIZE)
# define __mdlFinalize          mdlFinalize
#endif
...
#           if S_FUNCTION_LEVEL==2 && defined(MDL_FINALIZE)
              ssSetmdlFinalize(S,__mdlFinalize);
#           endif
...
#         if S_FUNCTION_LEVEL == 2 && defined(MDL_FINALIZE)
          case SS_CALL_MDL_FINALIZE:
            if (ssGetmdlFinalize(S) != NULL) {
                mdlFinalize(S);
            } else {
                ssSetErrorStatus(S,"Unexpected call to mdlFinalize");
            }
            break;
#         endif
